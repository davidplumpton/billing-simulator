...
- A learner overview page that links the major simulator workflows, explains the billing-data flow, exposes the fresh-start action, distinguishes safe start/reset/clone paths, and states the local synthetic safety boundaries.
...
- Billing pipeline persistence for usage events, metering records, priced bill line items, billing-period service summaries, support charges, basic EC2 Reserved Instance fee and coverage rows, basic Compute Savings Plan commitment fee/negation rows with coverage and amortized source links, month-end closes, issued bills, invoice obligations, and invoice documents.
- Savings Plans UI for creating simplified Compute Savings Plan purchases, viewing hourly/upfront commitments, generated fee and negation rows, covered source usage, and amortized source allocation.
...
- Scenario JSON/YAML parsing and execution for deterministic lab setup, including a Scenarios UI that lists and launches packaged "First consolidated bill", "Missing Tags", "Shared Networking allocation", "Payment Failure", "Forecast and Budget Alert", "Cost Explorer variance investigation", "Savings Plan coverage", and "Find the untagged data-transfer spike" labs with recent run audit state, reset-to-seed actions, workspace cloning, local review archive bundles containing the workspace database plus CUR-like export evidence, and a local scenario editor with validation preview.
