...
func (h costExplorerHandler) loadCostExplorerPageData(ctx context.Context, r *http.Request, data *costExplorerPageData) error {
	...
	data.Result = costExplorerResultViewFromResult(result, data.Builder.Metric)
	data.HasResult = true
	return nil
}
...
func costExplorerResultViewFromResult(result persistence.CostExplorerQueryResult, metric string) costExplorerResultView {
...
func costExplorerResultRowViewFromRow(row persistence.CostExplorerQueryRow, metric string) costExplorerResultRowView {
	...
	return costExplorerResultRowView{
		Period:       row.TimePeriodStart + " to " + row.TimePeriodEnd,
		Groups:       groups,
...
func costExplorerTables() costExplorerTablesView {
	return costExplorerTablesView{
		Results:      uiTable(uiTableHeaders("Period", "Groups", "Metric", "Usage", "Cost", "Items", "Currency"), "No report rows"),
		...
		{{else}}
			...
			<section>
				<div class="section-heading">
					...
					<span>{{.Result.MetricLabel}} / {{.Result.Granularity}}</span>
				</div>
					...
					<table class="dense-table">
						...
						<tbody>
							{{range .Result.Rows}}
								<tr>
									<td>{{.Period}}</td>
									<td>{{if .Groups}}<div class="tags">{{range .Groups}}<span>{{.}}</span>{{end}}</div>{{else}}<span class="muted">All spend</span>{{end}}</td>
