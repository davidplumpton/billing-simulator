...
- Scenario JSON/YAML parsing and execution for deterministic lab setup, including a Scenarios UI that lists and launches packaged "First consolidated bill", "Missing Tags", "Shared Networking allocation", "Payment Failure", "Forecast and Budget Alert", "Savings Plan coverage", and "Find the untagged data-transfer spike" labs with recent run audit state, reset-to-seed actions, workspace cloning, local review archive bundles containing the workspace database plus CUR-like export evidence, and a local scenario editor with validation preview.
