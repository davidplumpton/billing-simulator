...
- Bills UI with bill state summaries, charge breakdowns, resource-level charge rows, reconciliation data, printable HTML invoice pages, and invoice line-item CSV export; the reserved `/invoices/{invoice_id}/document.pdf` route currently returns a 501 placeholder describing the implementation plan rather than generated PDF output.
...
- FOCUS-style exports beyond the current CUR-like exports, plus richer query-lab workflows beyond the current external SQL examples.
